...
namespace some.reads.tech.Features.Bookshelves.Add_to_bookshelf
{
    public static class AddToBookshelf
    {
        ...
        {
            if (!BookExists(memoryCache, book.Query, book.Key))
        ...
        private static bool BookExists(IMemoryCache memoryCache, string query, string? key)
        {
            string searchCacheKey = $"books_search_{query}";

            if (memoryCache.TryGetValue(searchCacheKey, out OpenLibraryResponse<BookSearchResponse> cachedValue))
                return cachedValue.Docs.Any(x => x.Key.Equals(key));
